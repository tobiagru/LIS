...
param_grid = [{'n_neighbors': np.arange(3,20,dtype=np.int16),'leaf_size':np.arange(5,50,3,dtype=np.int16)},
              {'C': np.linspace(0,1,20)},
              {'C': np.linspace(0,1,20), 'gamma': np.linspace(0,5,20)},
              {'C': np.linspace(0,1,20), 'gamma': np.linspace(0,5,20), 'degree': range(2,4,1)},
              {'C': np.linspace(0,1,20), 'gamma': np.linspace(0,5,20)},
              {'max_depth': np.range(3,10,dtype=np.int16), 'max_features': np.arange(1,3,dtype=np.int16)},
              {'max_depth': np.range(3,15,dtype=np.int16), 'n_estimators': np.range(5,30,5,dtype=np.int16), 'max_features': np.arange(1,3,dtype=np.int16)},
              {'n_estimators': np.range(30,150,10,dtype=np.int16)},
              {},
              {},
              {}]
...
for name, clf, param in zip(names, classifiers, param_grid):
        ...
        if name is "Naive_Bayes" or name is "Linear_Discriminant_Analysis" or name is "Quadratic_Discriminant_Analysis":
            ...
            try:
                ...
                print('-> Problem while doing gridsearch with {0}'.format(name))
                clf_tmp = clf
                score = 0
                best_param = 0
            else:
        ...
        try:
            ...
            print('-> Problem while training with full training set on {0}'.format(name))

        try:
            y_pred = clf_tmp.predict(X_test)
        except:
            print('-> Problem while predicting on test set with {0}'.format(name))
        else:
